  ...
  return (
    <>
      <TableContainer className="table-container expandable-table-container">
        ...
        >
          <TableHead>
            <TableCell>Generated Group Id</TableCell>
            <TableCell>Options</TableCell>
          </TableHead>
          <TableBody>
            {generategroup?.map((l: GenerateGroupId) => (
              <TableRow key={l._id}>
                <TableCell>{l.groupId}</TableCell>
                <TableCell style={{ width: "5rem" }}>
                  <div className="display-flex align-center justify-end">
                    <SubjectAction Generategroups={l} />
                  </div>
                </TableCell>
              </TableRow>
            ))}
...
export interface ManageGenerateGroupActionProps {
  Generategroups: GenerateGroupId;
}

const SubjectAction: React.FC<ManageGenerateGroupActionProps> = (props) => {
  ...
  const [id, setId] = useState(props.Generategroups._id);
  ...
  const [remove, { status: removeStatus }] = useMutation(deleteNotAvailable, {
    onError() {
      ...
      displayToast(
        `Lecturer ${props.Generategroups.groupId} Removing Failed` || "Hi ",
        "default"
      );
    },
    onSuccess() {
      displayToast(
        `Lecturer ${props.Generategroups.groupId}  Removed`,
        "default"
      );
    },
  });
                ...
                <form onSubmit={handleSubmit(onSubmit)} noValidate={false}>
                  <Grid container spacing={2} className="form-row">
                    ...
                    <input
                      ...
                      id="typeId"
                      name="typeId"
                      value={props.Generategroups._id}
                      ref={register}
                    />
                    <input
                      type="hidden"
                      id="name"
                      name="name"
                      value={props.Generategroups.groupId}
                      ref={register}
                    />
                    <Grid item xs={6}>
                      <div>
